...
interface CorsoROAPageProps {
  onNavigate: (view: any) => void;
  onOpenCourse: (courseId: string) => void;
}

const CorsoROAPage: React.FC<CorsoROAPageProps> = ({ onNavigate, onOpenCourse }) => {
  const course = courses.find(c => c.id === "41");

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Corso Radiazioni Ottiche Artificiali (ROA) | Formazione E-learning Sicurezza";
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute('content', "Corso di formazione online sul rischio da Radiazioni Ottiche Artificiali (ROA). Effetti su occhi e pelle, DPI specifici e misure di prevenzione. Formazione certificata.");
    }
  }, []);

  const mailtoLink = "mailto:commerciale@aliseogroup.it?subject=Richiesta%20informazioni%20corso%20Radiazioni%20Ottiche%20Artificiali&body=Buongiorno%2C%0A%0ASono%20interessato%20al%20corso%20e-learning%20sulle%20Radiazioni%20Ottiche%20Artificiali%20(ROA).%0APotrei%20avere%20maggiori%20informazioni%3F%0A%0AGrazie.";
      ...
      <section className="relative pt-20 pb-16 bg-slate-900 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl">
            ...
            >
              Corso <span className="text-brand">Radiazioni Ottiche Artificiali</span> (ROA)
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-6">
            Necessiti di formazione sul rischio ROA?
          </h2>
          <p className="text-lg text-slate-600 mb-10">
            Il nostro team è a disposizione per supportarti nella valutazione dei rischi e nella scelta dei DPI più idonei per la tua attività.
          </p>
      ...
      <section className="py-16 border-t border-slate-100 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: "Campi Elettromagnetici", desc: "Rischi legati a macchinari industriali e trasmettitori", view: 'corso-campi-elettromagnetici', icon: Zap },
